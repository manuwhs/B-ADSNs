...
#        print W
        ...
        O = (O + Ol * self.gammas[l])/(1+self.gammas[l]);       # update strong classifier
        Oval = (Oval + OlVal * self.gammas[l])/(1+self.gammas[l])
...
def check_stop_L(self, l):
    ...
    if (self.Enphasis == "NeoBoosting"):
        ...
        if (method == 0):
            if (l >= 5):    ### SETS THE MINIMUM AMOUNT OF LAYERS !!
                n_gammas = 2
...
#            print self
        ...
        O += (self.layers[l].fo(Zo) * self.gammas[l])/(1+self.gammas[l])
        O = self.layers[l].fh(O)/(1+self.gammas[l])
#        print output
#        print self.alphas[t]
    return O
