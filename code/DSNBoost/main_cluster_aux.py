...
if (Cluster_exec == 1): # If executed using command line from root folder.
    ...
    output_folder = "/export/clusterdata/mmontoya/ResultsNeoDSNaux/"
...
L = 200         # Maximum number of layers 
...
visual_DSNBoost = DSNBoost.DSN_visual(verbose = 1,
...
if ((Cluster_exec == 1)|(Console_exec == 1)):
    ...
    N_epochs_list = [50,100, 200,300,500]
    fo_list = ["linear","tanh"]
    
    Learning_Rate_list = [0.01,0.005,0.002,0.001]
    BatchSize_list = [1,3,4,5,10,20]
    
    Inyection_list =  ["WholeO",   "PrevZo" ]
    Enphasis_list = [ "RealAdaBoost","NeoBoosting"] 
...
if (BP_F == 1):
    ...
    trainingAlg = paC.Training_Alg("BP",[N_epochs,Learning_Rate, 0.0])
