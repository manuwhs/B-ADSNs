...
def PPE_alpha_betaTr (PPE_p, main_folder, database_number,Ninit_i,Roh_i, nH_i_list, Nepoch_i_list):
    ...
    for Nepoch_i in  Nepoch_i_list :
        for nH_i in nH_i_list:
            ...
            for alpha_i in range(Na):
                ...
                for beta_i in range(Nb):
                    pene = All_ave_val[beta_i][alpha_i][Ninit_i][Roh_i][Nepoch_i][nH_i]
                    Ave_val_ab[aux].append(pene)   
                    pene = All_ave_tr[beta_i][alpha_i][Ninit_i][Roh_i][Nepoch_i][nH_i]
        ...
        #    base_folder_out = base_folder_out   # + "b: "+ str(beta_i) + "/"
            ...
            mg.plot_3D_a_nH(PPE_p.alpha_list[PPE_p.alpha_indx_list], PPE_p.beta_list[PPE_p.beta_indx_list], Ave_tr_ab)
            ...
            mg.plot_3D_a_nH(PPE_p.alpha_list[PPE_p.alpha_indx_list], PPE_p.beta_list[PPE_p.beta_indx_list], Ave_val_ab)
            ...
            plt.savefig(base_folder_out + "Ave_nLs(nH)_nH:" + str(PPE_p.nH_list[PPE_p.nH_indx_list[nH_i]]) + " "
                        + str(PPE_p.N_epochs_list[PPE_p.N_epochs_indx_list[Nepoch_i]])+".png")
            plt.close("all")
            
            mg.plot_acc_nL_nH(PPE_p.beta_list[PPE_p.beta_indx_list], Ave_val_ab.T)
            plt.savefig(base_folder_out + "Ave_nLls(nH)_nH:" + str(PPE_p.nH_list[PPE_p.nH_indx_list[nH_i]]) + " "
                        + str(PPE_p.N_epochs_list[PPE_p.N_epochs_indx_list[Nepoch_i]])+".png")
            plt.close("all")
