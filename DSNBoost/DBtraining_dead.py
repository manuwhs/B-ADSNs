...
#        print W
        ...
        if (l > 0):
            Xlayer_fin = np.concatenate((Xtrain,np.zeros(O.shape)), axis = 1)  # Build training data
            XlayerVal_fin = np.concatenate((Xval,np.zeros(Oval.shape)), axis = 1)  # Build validation data
            
            # Set Previous output for the next layer
            Zo = layer.get_Zo(Xlayer_fin)
            ZoVal = layer.get_Zo(XlayerVal_fin)
    ...
    for l in range (lEnd):
        if (l == 0):
            ...
            if (self.Inyection == "PrevZo"):
                ...
                Xlayer = np.concatenate((X,np.zeros(O.shape)),axis = 1) 
